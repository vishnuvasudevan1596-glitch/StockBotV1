...
def send_telegram(message):
    print(f"Telegram send: {message[:50]}...", flush=True)
    if not CONFIG["TELEGRAM_TOKEN"] or not CONFIG["TELEGRAM_CHAT_ID"]:
        print("ERROR: Missing TELEGRAM_TOKEN or CHAT_ID", flush=True)
        return
    ...
    try:
        ...
        if r.status_code != 200:
            print(f"Telegram API error: {r.text}", flush=True)
...
def simulate_trade(signal):
    ...
    print(f"[SIMULATION] Opened {signal['symbol']} BUY @ {signal['entry']} | SL {signal['sl']} | Target {signal['target']}", flush=True)
...
if __name__ == "__main__":
    print("DEBUG: Reading environment variables", flush=True)
    CONFIG["TELEGRAM_TOKEN"] = os.getenv("TELEGRAM_TOKEN")
    CONFIG["TELEGRAM_CHAT_ID"] = os.getenv("TELEGRAM_CHAT_ID")
    print(f"DEBUG: TOKEN set = {bool(CONFIG['TELEGRAM_TOKEN'])}", flush=True)
    print(f"DEBUG: CHAT_ID set = {bool(CONFIG['TELEGRAM_CHAT_ID'])}", flush=True)
    try:
        main_loop()
    except Exception as e:
